...
input_terms_directory = Path("_archive/JSONLD/auxillary/product/")
output_terms_directory = Path("datadescriptor/product/")

os.makedirs(output_terms_directory, exist_ok=True)

terms_file_list = [p for p in input_terms_directory.iterdir() if (p.suffix==".json" and "jsonld" not in str(p))]
